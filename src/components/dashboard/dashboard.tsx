...
import { getVisitorHistory } from "@/apis/visitorApi"; // New API function
...
const Dashboard = () => {
    ...
    // State for feedback data
    const [feedbacks, setFeedbacks] = useState<any[]>([]);
    const [visitorHistory, setVisitorHistory] = useState<any[]>([]); // New state for visitor history
    const [loading, setLoading] = useState(true);

    // Fetch feedback data and visitor history
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const [feedbackData, visitorData] = await Promise.all([
                    getAllFeedback(),
                    getVisitorHistory() // New API call
                ]);
    ...
    // Generate chart data based on view and real data
    const chartData = useMemo(() => {
        ...
        if (view === "week") {
            ...
            return days.map(day => {
                ...
                const dayVisitors = visitorHistory.find(v => v.date === dayStr)?.count || 0;
                
                // Get feedbacks for this day
                const dayFeedbacks = feedbacks.filter(f => {
                    const feedbackDate = f.created_at?.split('T')[0];
                    return feedbackDate === dayStr;
                });
                
                const feedbackCount = dayFeedbacks.length;
                const avgRating = feedbackCount > 0
                    ? dayFeedbacks.reduce((sum, f) => sum + Number(f.rating), 0) / feedbackCount
                    : 0;
            ...
            return months.map(month => {
                ...
                const monthVisitors = visitorHistory
                    .filter(v => v.date.startsWith(monthStr))
                    .reduce((sum, v) => sum + v.count, 0);
                
                // Get feedbacks for this month
                const monthFeedbacks = feedbacks.filter(f => {
                    const feedbackDate = f.created_at?.split('T')[0];
                    return feedbackDate?.startsWith(monthStr);
                });
                
                const feedbackCount = monthFeedbacks.length;
                const avgRating = feedbackCount > 0
                    ? monthFeedbacks.reduce((sum, f) => sum + Number(f.rating), 0) / feedbackCount
                    : 0;
        ...
        return [];
    }, [view, feedbacks, visitorHistory, loading]);
