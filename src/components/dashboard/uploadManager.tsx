// src\components\dashboard\uploadManager.tsx
import React, { useState, useEffect, useRef } from "react";
import {
    X, Play, Pause, Trash2, Upload, Volume2, VolumeX, PlusCircle,
...
const UploadManager: React.FC = () => {
    ...
    // --- Keyword Management ---
    const handleAddKeyword = (artifactId: string) => {
        if (currentKeyword.trim()) {
            setArtifacts(prev =>
                prev.map(artifact => {
                    if (artifact.id === artifactId) {
                        if (!artifact.keywords.includes(currentKeyword.trim())) {
                            return {
                                ...artifact,
                                keywords: [...artifact.keywords, currentKeyword.trim()],
                            };
                        }
                    }
                    return artifact;
    ...
    const handleRemoveKeyword = (artifactId: string, keyword: string) => {
        setArtifacts(prev =>
            prev.map(artifact => {
                if (artifact.id === artifactId) {
                    return {
                        ...artifact,
                        keywords: artifact.keywords.filter(k => k !== keyword),
                    };
                }
                return artifact;
            })
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, artifactId: string) => {
        if (e.key === "Enter") {
            e.preventDefault();
            handleAddKeyword(artifactId);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        for (const artifact of artifacts) {
            ...
            if (!artifact.name || !artifact.category || artifact.keywords.length === 0 || !artifact.imageUrl) {
        ...
        try {
            for (const artifact of artifacts) {
                ...
                const artifactData: any = {
                    ...
                    has_audio: artifact.audioFiles.length > 0, // Add this
                };
                ...
                artifact.audioFiles.forEach(file => {
                    if (file.backendUrl) {
                        if (file.language === 'English') {
                            artifactData.english_audio_url = file.backendUrl;
                        } else if (file.language === 'Hindi') {
                            artifactData.hindi_audio_url = file.backendUrl;
                        } else if (file.language === 'Assamese') {
                            artifactData.assamese_audio_url = file.backendUrl;
                        }
                    }
                });

                // Save to database
                const response = await saveArtifact(artifactData);

                // Update with audio_guide_id
                if (response && response.id) {
                    await updateArtifact(response.id, {
                        audio_guide_id: response.id
                    });
                }
            }
            ...
            triggerToast("Failed to save artifacts", "error");
        }
    };
        ...
        <div className="w-full min-h-screen py-8 sm:py-4 bg-gray-100 dark:bg-zinc-900 text-zinc-900 dark:text-white">
            ...
            <div className="max-w-6xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-3xl font-bold">Artifact & Audio Manager</h2>
                    <button
                        type="button"
                        onClick={addArtifact}
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    {artifacts.map((artifact, index) => (
                        ...
                        >
                            ...
                            {/* Image Upload */}
                            <div className="mb-6">
                                <label className="block mb-2 font-semibold">Artifact Image</label>
                                <UploadCollectionsImage
                                    onUploadSuccess={(url) => {
                                        setArtifacts(prev =>
                                            prev.map(a =>
                                                a.id === artifact.id ? { ...a, imageUrl: url } : a
                                            )
                                        );
                                    }}
                                />
                                ...
                                <div className="mt-3">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setArtifacts(prev =>
                                                prev.map(a =>
                                                    a.id === artifact.id ? { ...a, imageUrl: e.target.value } : a
                                                )
                                            )
                                        }
                                        placeholder="Image URL will appear here after upload"
                                        className="w-full px-3 py-2 text-sm rounded-md border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600"
                                        required
                                    />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setArtifacts(prev =>
                                                prev.map(a =>
                                                    a.id === artifact.id ? { ...a, name: e.target.value } : a
                                                )
                                            )
                                        }
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) =>
                                            setArtifacts(prev =>
                                                prev.map(a =>
                                                    a.id === artifact.id ? { ...a, category: e.target.value } : a
                                                )
                                            )
                                        }
                                        className="w-full px-4 py-2 rounded-md border bg-white dark:bg-zinc-700 border-gray-300 dark:border-zinc-600 focus:ring-2 focus:ring-red-800 dark:focus:ring-amber-600 focus:outline-none"
                                ...
                                <div className="md:col-span-2">
                                    <label className="block mb-2 font-medium">
                                        Artifact Keywords *
                                        <span className="text-gray-500 dark:text-gray-400 text-xs ml-2">(press enter to add)</span>
                                    </label>
                                        ...
                                        {artifact.keywords.map(keyword => (
                                            ...
                                            >
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => handleRemoveKeyword(artifact.id, keyword)}
                                                >
                                                    <X size={16} />
                                                </button>
                                            </span>
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            className="flex-1 px-4 py-2 rounded-md border bg-white dark:bg-zinc-700 border-gray-300 dark:border-zinc-600 focus:ring-2 focus:ring-red-800 dark:focus:ring-amber-600 focus:outline-none"
                                        />
                                        <button
                                            type="button"
                                            onClick={() => handleAddKeyword(artifact.id)}
                                            className="px-4 py-2 bg-red-800 dark:bg-amber-600 text-white rounded-md"
                                        >
                                            Add
                                        </button>
                                    </div>
                                                ...
                                                {file ? (
                                                    ...
                                                    <label className="block cursor-pointer">
                                                        ...
                                                        <input
                                                            type="file"
                                                            accept="audio/*"
                                                            onChange={e => handleAudioUpload(e, language, artifact.id)}
                                                            className="hidden"
                                                        />
                                                    </label>
                                                )}
                            ...
                            {index === artifacts.length - 1 && (
                                <div className="mt-6 pt-4 border-t border-gray-200 dark:border-zinc-700">
                                    ...
                                    <div className="flex justify-end">
                                        <button
                                            type="submit"
                                            className="bg-red-800 hover:bg-red-900 dark:bg-amber-600 dark:hover:bg-amber-700 text-white px-6 py-3 rounded-lg font-medium transition-colors flex items-center gap-2"
                                        >
                                            <Upload size={18} />
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`w-0.5 rounded-full transition-all duration-300 ${isActive
                                                    ...
                                                    : 'bg-stone-950 dark:bg-zinc-100 opacity-30'
                                                    }`}
                                                style={{
