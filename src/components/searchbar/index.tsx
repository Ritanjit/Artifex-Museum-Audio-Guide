...
const FloatingSearchBar: React.FC<FloatingSearchBarProps> = ({ searchQuery, setSearchQuery, handleSearch, showPrompt = false }) => {
    ...
    return (
        <div className="relative flex flex-col items-center mt-6 px-4 w-full">
            ...
            <div className="relative flex items-center bg-white/10 backdrop-blur-lg shadow-md 
            ...
            h-[50px] sm:h-[50px] md:h-[60px] p-2 pl-4 pr-10 sm:pr-16 md:pr-20">
                ...
                <input
                    ...
                    placeholder="Search for artifacts & more..."
            ...
            {filteredSuggestions.length > 0 && (
                <ul className="absolute top-[55px] sm:top-[60px] bg-white/10 text-white w-full max-w-[600px] 
                rounded-lg shadow-lg mt-2 py-2 z-50 max-h-[200px] overflow-y-auto backdrop-blur-3xl text-left">
                    {filteredSuggestions.slice(0, 5).map((suggestion, index) => (
                        <li 
                            key={index} 
                            className="px-4 py-2 cursor-pointer hover:bg-gray-950/30 transition text-sm"
            ...
            {/* Google-like Search Prompt */}
            {showPrompt && filteredSuggestions.length === 0 && searchQuery && (
                <div className="absolute top-[70px] sm:top-[70px] bg-white text-gray-700 px-4 py-2 
                rounded-md shadow-md text-sm border border-gray-300 w-[280px] sm:w-[250px] text-center">
                    Press <span className="font-semibold">Enter</span> to search for "<span className="text-black font-medium">
                        {searchQuery}</span>"
                </div>
            )}
